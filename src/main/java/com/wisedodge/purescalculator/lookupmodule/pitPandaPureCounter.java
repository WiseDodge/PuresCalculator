...
public class pitPandaPureCounter {
  ...
  public static Map<String, Integer> countItems(JSONObject pitPandaResponse) {
    ...
    JSONObject inventoriesObject = pitPandaResponse.getJSONObject("data").getJSONObject(
            "inventories");
        ...
        if (item.has("name")) {
          ...
          switch (itemName) {
            ...
            case "§aPhilosopher's Cactus":
              ...
              break;
            case "§bPants Bundle":
              if (item.getJSONArray("desc").getString(item.getJSONArray("desc").length()
                      - 1).equals("open")) {
            ...
            case "§dMini Cake":
              ...
              break;
            // ? Might add more cases for other items
            default:
              System.out.println("How did you end up here?");
          }

          // Logging item count
          com.wisedodge.purescalculator.logging.devLogger.logItemCount("Item: %s, Count: %d%n", itemName, itemCount);
        }
      }
    }
  ...
  public static double calculateTotalNetWorth(Map<String, Integer> itemCounts) {
    ...
    double totalNetWorthInPures =
            Math.round(totalNetWorthInVile / pantsBundleToVileRate * 100.0) / 100.0;

    // Logging total net worth
    com.wisedodge.purescalculator.logging.devLogger.logTotalNetworth("Total Net Worth in Pures: %.2f%n", totalNetWorthInPures);
